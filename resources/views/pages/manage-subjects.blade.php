    ...
    $.post('{{ route("api.subjects") }}',{courseId:courseId},function(res){
        if(res.status===200){
            ...
            if(!res.data.length) html='<tr><td colspan="6" class="text-center">No subjects found</td></tr>';
                ...
                html+='<tr><td>'+(i+1)+'</td><td>'+escapeHtml(s.title)+'</td><td>'+(s.code||'-')+'</td><td>'+escapeHtml(s.description||'-')+'</td><td><span class="badge badge-active">Active</span></td><td><button class="btn btn-sm btn-danger" onclick="deleteSubject('+s.id+')"><i class="fas fa-trash"></i></button></td></tr>';
            });
